...
"""
modification performed over: https://gist.github.com/ales-erjavec/7624dd1d183dfbfb3354600b285abb94
"""
...
class CheckComboBox(QComboBox):
    ...
    def switch_all_selection(self):
        def all_selected():
            for i in range(self.count()):
                if self.itemCheckState(i) == Qt.Unchecked:
                    return False
            return True
    ...
    def select_all(self):
        for i in range(self.count()):
            self.setItemCheckState(index=i, state=Qt.Checked)

    def deselect_all(self):
        for i in range(self.count()):
            self.setItemCheckState(index=i, state=Qt.Unchecked)
